...
# ==========================================
# HEADER & AUTO-REFRESH LOGIK
# ==========================================
col_head, col_status = st.columns([7, 3])
...
with col_status:
    ...
    # Der Bearbeitungs-Schalter
    edit_mode = st.toggle("✏️ Bearbeitungsmodus", value=False, help="Pausiert das Live-Laden, damit du in Ruhe Tabellen bearbeiten kannst.")
    
    if edit_mode:
        st.warning("⏸️ Auto-refresh inaktiv")
    else:
        st.success("✅ Autorefresh aktiv (30s)")
        # Löst alle 30.000 Millisekunden einen Reload aus
...
with col1:
    st.markdown("<div style='text-align: center;'><strong>1 - Sägemehl</strong></div>", unsafe_allow_html=True)
    val_sm = svs.vertical_slider(key="vs_sm", default_value=st.session_state.get("bunker_sm", 50), step=10, min_value=0, max_value=100, slider_color="#2e7d32", track_color="#dcdcdc")
    if val_sm is not None and val_sm != st.session_state.bunker_sm:
        st.session_state.bunker_sm = val_sm
        save_persistent_data()
    
    if st.session_state.bunker_sm <= 10: st.warning("⛔ GESPERRT")
    elif st.session_state.bunker_sm >= 80: st.error("🚨 HOCH")
    else: st.success("✅ Normal")

with col2:
    st.markdown("<div style='text-align: center;'><strong>2 - Hackschnitzel</strong></div>", unsafe_allow_html=True)
    val_hs = svs.vertical_slider(key="vs_hs", default_value=st.session_state.get("bunker_hs", 50), step=10, min_value=0, max_value=100, slider_color="#2e7d32", track_color="#dcdcdc")
    if val_hs is not None and val_hs != st.session_state.bunker_hs:
        st.session_state.bunker_hs = val_hs
        save_persistent_data()
        
    if st.session_state.bunker_hs <= 10: st.warning("⛔ GESPERRT")
    elif st.session_state.bunker_hs >= 80: st.error("🚨 HOCH")
    else: st.success("✅ Normal")

with col3:
    st.markdown("<div style='text-align: center;'><strong>3 - Rinde</strong></div>", unsafe_allow_html=True)
    val_ri = svs.vertical_slider(key="vs_ri", default_value=st.session_state.get("bunker_ri", 50), step=10, min_value=0, max_value=100, slider_color="#2e7d32", track_color="#dcdcdc")
    if val_ri is not None and val_ri != st.session_state.bunker_ri:
        st.session_state.bunker_ri = val_ri
        save_persistent_data()
        
    if st.session_state.bunker_ri <= 10: st.warning("⛔ GESPERRT")
    elif st.session_state.bunker_ri >= 80: st.error("🚨 HOCH")
    else: st.success("✅ Normal")

with col4:
    st.markdown("<div style='text-align: center;'><strong>4 - Kappholz</strong></div>", unsafe_allow_html=True)
    val_kp = svs.vertical_slider(key="vs_kp", default_value=st.session_state.get("bunker_kp", 50), step=10, min_value=0, max_value=100, slider_color="#2e7d32", track_color="#dcdcdc")
    if val_kp is not None and val_kp != st.session_state.bunker_kp:
        st.session_state.bunker_kp = val_kp
        save_persistent_data()
        
    if st.session_state.bunker_kp <= 10: st.warning("⛔ GESPERRT")
    elif st.session_state.bunker_kp >= 80: st.error("🚨 HOCH")
    else: st.success("✅ Normal")

st.divider()

# ==========================================
# WORKSPACES (Tabs statt vertikalem Scrollen)
# ==========================================
tab_dispo, tab_manuell, tab_stammdaten, tab_historie = st.tabs([
    "📅 Disposition & Kalender", 
    "🛠️ Manuelle Buchungen", 
    "⚙️ Stammdaten & Kontingente", 
    "📜 Logbuch"
])

# ------------------------------------------
# TAB 1: DAS TAGESGESCHÄFT (DISPO)
# ------------------------------------------
with tab_dispo:
    # 1. PLANUNGS-ALGORITHMUS INKL. SCORE-BERECHNUNG
    bunker_levels = {
    ...
    # Globale Variablen für den Algorithmus holen
    ...
    # Kundensperren erfassen für den Algorithmus
    ...
    # Kurz temporär die aktuellen Sperren auslesen
    selected_blocked_custs = st.session_state.get(f"block_cust_{selected_day}", [])
...
# ------------------------------------------
# TAB 2: MANUELLE BUCHUNGEN
# ------------------------------------------
with tab_manuell:
    st.markdown("### 🚛 Fremdspeditionen-Terminal (Manuelle Liste)")
...
with tab_stammdaten:
    ...
    st.markdown("### 📋 Wochen-Kontingente Interner Fuhrpark")
    ...
    selected_blocked_custs_ui = st.multiselect(
